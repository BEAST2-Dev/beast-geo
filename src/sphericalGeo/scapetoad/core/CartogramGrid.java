 ...
 */
public class CartogramGrid implements Serializable
{
	...
	throws InterruptedException
	{
		...
		while (featIter.hasNext())
		{
			...
			AppContext.getStatusTracker().updateRunningStatus(progress,
				"Computing the density for the cartogram grid...",
				"Preparing feature " + (featCnt+1) + " of " + nFeat);
	...
	public void updateDensityValues ()
	{
		...
		for (j = 0; j < (mGridSizeY - 1); j++)
		{
			for (i = 0; i < (mGridSizeX - 1); i++)
			{
				...
				double currentArea = 
					sphericalGeo.scapetoad.core.Geometry.areaOfQuadrangle(
	...
	public double[] projectPoint (double x, double y)
	{
		...
		double[] s = sphericalGeo.scapetoad.core.Geometry.intersectionOfSegments(
