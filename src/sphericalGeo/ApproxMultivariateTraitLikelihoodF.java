...
public class ApproxMultivariateTraitLikelihoodF extends GenericTreeLikelihood implements StateNodeInitialiser, LocationProvider { 
	...
	public void initAndValidate() {
		...
		List<GeoPrior> geopriors = multiGeopriorsInput.get().geopriorsInput.get();
		{
			...
			if (ids.length() > 1) {
				...
				Log.warning.println("\nWARNING: this analysis contains GeoPriors (" + ids + "), but these are not "
						+ "connected to the MultiGeoPrior (" + getID() + ").");
...
//		initialiseSampledStates();
	...
	void initialiseSampledStates() {
		...
		MultiGeoPrior multiGeoprior = multiGeopriorsInput.get();
		if (isSampled == null) {
			...
			taxonNrs = new int[multiGeoprior.size()];
			storedTaxonNrs = new int[multiGeoprior.size()];
			Arrays.fill(storedTaxonNrs, -1);
		}
		Arrays.fill(isSampled, false);
		//sampleNumber.clear();
		Arrays.fill(taxonNrs, 0);
		
		if (multiGeoprior.size() > 0) {
			...
			if (multiGeoprior.isMonoPhyletic()) {
				multiGeoprior.requiresRecalculation();
				try {
					multiGeoprior.calculateLogP();
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				for (int k = 0; k < multiGeoprior.size(); k++) {
					int taxonNr = multiGeoprior.getCladeTopNodeNr(k);
//					if (taxonNr != taxonNr2) {
//						int h = 3;
//						h++;
//						int taxonNr3 = multiGeoprior.getCladeTopNodeNr(k);
//						System.err.print(taxonNr + " " + taxonNr2 + " " + taxonNr3);
//					}
					isSampled[taxonNr] = true;
					//sampleNumber.add(taxonNr);
					int storedTaxonNr = storedTaxonNrs[k];
					if (storedTaxonNr >= 0) {
						...
						double [] location = multiGeoprior.getPrior(k).sample();
...
//					if (k == 4) {
//						System.err.print(storedTaxonNrs[k] + " <= " + taxonNr);
//					}
					taxonNrs[k] = taxonNr;
				}
				multiGeoprior.requiresRecalculation();
		...
		try {
			if (multiGeoprior.size() > 0) {
				loggerLikelihood.initByName("scale", scaleByBranchLength, "tree", tree, "siteModel", siteModel, 
					...
					"geoprior", multiGeoprior.geopriorsInput.get(), "location", sampledLocations, "transformer", transformer);
			} else {
				loggerLikelihood.initByName("scale", scaleByBranchLength, "tree", tree, "siteModel", siteModel, 
	...
	void sanitycheck() {
		...
		for (int i : taxonNrs /*sampleNumber*/) {
			...
			GeoPrior prior=  multiGeopriorsInput.get().getPrior(k);
