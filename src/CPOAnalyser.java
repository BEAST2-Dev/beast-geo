...
public class CPOAnalyser extends Runnable {
	...
	public void run() throws Exception {
		...
		TreeSet treeSet = getTreeSet(mcmc);
    	treeSet.reset();
		...
		double LPML = calcLPML(order, minLogP, cpoTable, weights);
    	...
    	while (k < replicates) {
    		...
    		LPMLs[k] = calcLPML(order, minLogP, cpoTable, weights);
	...
	private MCMC getMCMC() throws SAXException, IOException, ParserConfigurationException, XMLParserException {
		MCMC mcmc = null;
	...
	private TreeSet getTreeSet(MCMC mcmc) throws IOException {
		TreeSet treeSet = null;
		if (treeFileInput.get() != null) {
			treeSet = new MemoryFriendlyTreeSet(treeFileInput.get().getPath(), burninInput.get());		
		} else {
			for (Logger logger : mcmc.loggersInput.get()) {
				if (!logger.isLoggingToStdout() && logger.modeInput.get().equals(Logger.LOGMODE.tree)) {
					if (treeSet != null) {
						...
						treeSet = new MemoryFriendlyTreeSet(logger.fileNameInput.get(), burninInput.get());
					}
				}
			}
			if (treeSet == null) {
	...
	private LogAnalyser getTraceLog(MCMC mcmc) throws IOException {
		LogAnalyser tracelog = null;
		if (traceFileInput.get() != null) {
	...
	private double calcLPML(int [] order, double[] minLogP, CPOTable cpoTable, int[] patternWeights) {
		int patternCount = cpoTable.patterLogProbs.length;
		int treeSetSize = cpoTable.patterLogProbs[0].length;
  		// Equation (14) in Lewis et al 2014
    	double [] CPOi = new double[patternCount];
  		for (int i = 0; i < patternCount; i++) {
    		double CPOi_ = Math.log(treeSetSize) + minLogP[i];
    		double sum = 0;
    		double [] p = cpoTable.patterLogProbs[i];
        	for (int k : order) {
